export const BASE_URL = "http://marinambur.students.nomoreparties.xyz";
...
export const getToken = (token) => {
  return fetch(`${BASE_URL}/users/me`, {
    ...
    .then((res) => {
      ...
      return res
        .json()
        .then((data) => Promise.reject(`${res.status} - ${data.error}`));
    })
    ...
    .then((res) => {
      ...
      return Promise.reject(
        res.status === 400
          ...
          : `Пользователь с таким email не найден ${res.status}`
      );
    })
    .then((data) => {
      if (data.token) {
