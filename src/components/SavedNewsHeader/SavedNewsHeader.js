...
function SavedNewsHeader(props) {
  ...
  const getCountArticles = () => {
    api.getAllArticles()
      .then((res) => {
        ...
        setUniqueNames(keywordArray.reduce((y, x) => y.includes(x) ? y : [...y, x], []).sort());
      })
  ...
  return (
    <div className='saved-news-header'>
      ...
      <p className='saved-news-header__keywords'>По ключевым словам: <span className='saved-news-header__keywords_bold'>{uniqueNames.length === 0 ? '' : `${uniqueNames[0]}, ${uniqueNames[1] ? uniqueNames[1] : ''} и ${uniqueNames.length - 2 || 0} другим.`}</span></p>
    </div>
  );
}
