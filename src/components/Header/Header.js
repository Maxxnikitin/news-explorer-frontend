...
function Header(props) {
  ...
  const menuButtonClassName = (
    `${(props.theme === 'white') ? 'header__menu-button-img header__menu-button-img_theme_white' : 'header__menu-button-img'}`
  );
  return (
    <header className='header'>
      ...
      <section className={`header__popup-mobile ${props.menuIsOpen && 'header__popup-mobile_opened'}`}>
          <div className='header__overlay'></div>
          <div className='header__mobile-menu'>
            <Navigation place='mobile' theme='white' activeClass={props.activeClass} onClick={props.onClick} isLogged={props.isLogged} />
          </div>
        </section>
    </header>
  );
